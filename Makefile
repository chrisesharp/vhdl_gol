# Makefile automatically generated by ghdl
# Version: GHDL 0.33 (20150921) [Dunoon edition] - llvm code generator
# Command used to generate this makefile:
# ghdl --gen-makefile cell_testbench

GHDL=ghdl
GHDLFLAGS=

# Default target
all: cell_testbench run

# Elaboration target
cell_testbench: /usr/local/lib/ghdl/v93/ieee/std_logic_1164.o /usr/local/lib/ghdl/v93/ieee/std_logic_1164_body.o /usr/local/lib/ghdl/v93/ieee/numeric_std.o /usr/local/lib/ghdl/v93/ieee/numeric_std-body.o cell.o cell_testbench.o
	$(GHDL) -e $(GHDLFLAGS) $@

# Run target
run: cell_testbench
	$(GHDL) -r cell_testbench $(GHDLRUNFLAGS)

# Targets to analyze files
/usr/local/lib/ghdl/v93/ieee/std_logic_1164.o: /usr/local/lib/ghdl/v93/ieee/../../src/ieee/std_logic_1164.v93
	@echo "This file was not locally built ($<)"
	exit 1
/usr/local/lib/ghdl/v93/ieee/std_logic_1164_body.o: /usr/local/lib/ghdl/v93/ieee/../../src/ieee/std_logic_1164_body.v93
	@echo "This file was not locally built ($<)"
	exit 1
/usr/local/lib/ghdl/v93/ieee/numeric_std.o: /usr/local/lib/ghdl/v93/ieee/../../src/ieee/numeric_std.v93
	@echo "This file was not locally built ($<)"
	exit 1
/usr/local/lib/ghdl/v93/ieee/numeric_std-body.o: /usr/local/lib/ghdl/v93/ieee/../../src/ieee/numeric_std-body.v93
	@echo "This file was not locally built ($<)"
	exit 1
cell.o: cell.vhdl
	$(GHDL) -a $(GHDLFLAGS) $<
cell_testbench.o: cell_testbench.vhdl
	$(GHDL) -a $(GHDLFLAGS) $<

# Files dependences
/usr/local/lib/ghdl/v93/ieee/std_logic_1164.o:
/usr/local/lib/ghdl/v93/ieee/std_logic_1164_body.o:  /usr/local/lib/ghdl/v93/ieee/std_logic_1164.o
/usr/local/lib/ghdl/v93/ieee/numeric_std.o:  /usr/local/lib/ghdl/v93/ieee/std_logic_1164.o
/usr/local/lib/ghdl/v93/ieee/numeric_std-body.o:  /usr/local/lib/ghdl/v93/ieee/numeric_std.o
cell.o:  /usr/local/lib/ghdl/v93/ieee/std_logic_1164.o /usr/local/lib/ghdl/v93/ieee/numeric_std.o
cell_testbench.o:  /usr/local/lib/ghdl/v93/ieee/std_logic_1164.o /usr/local/lib/ghdl/v93/ieee/std_logic_1164_body.o /usr/local/lib/ghdl/v93/ieee/numeric_std.o /usr/local/lib/ghdl/v93/ieee/numeric_std-body.o cell.o
